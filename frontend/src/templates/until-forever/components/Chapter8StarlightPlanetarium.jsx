...
import { Heart, Sparkles, MapPin, Moon } from "lucide-react";

/**
 * Micro Sound Synthesizer for Starlight Connections
 */
...
/**
 * Chapter 8: The Starlight Planetarium (Interactive Constellations)
 * -----------------------------------------------------------------
 * Emotion: Calmness, Peace & Quiet Reflection.
 * Gentle breathing stars, constellation line connection, and quiet memory unfolding.
 */
export default function Chapter8StarlightPlanetarium({ content = {}, onComplete }) {
  const constellations = [
    {
      ...
      icon: "❤️",
      title: "First Meeting",
      subtitle: "The Spark Under the Stars",
    ...
    },
    {
      ...
      note: "3 hours of talking about everything and nothing. We fell asleep with the phone still connected.",
      image: "https://images.unsplash.com/photo-1494232410401-ad00d5433cfa?auto=format&fit=crop&w=800&q=80",
    },
    {
      id: "unforgettable-trip",
      icon: "🚆",
      title: "The Trip We Never Forgot",
    ...
    {
      id: "cafe-changed-everything",
      icon: "☕",
      title: "The Café Where Everything Changed",
  ...
  const isCurrentCompleted = completedConstellations.includes(currentConstellation.id);

  const handleConnectConstellation = () => {
    playPlanetariumSound("starConnect");

    if (!isCurrentCompleted) {
      setCompletedConstellations((prev) => [...prev, currentConstellation.id]);
    }
  };
  ...
  return (
    <div className="relative min-h-screen bg-[#030208] text-rose-100 flex flex-col items-center justify-between p-4 sm:p-8 overflow-hidden select-none font-serif">
      ...
      <motion.div
        ...
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[650px] h-[650px] bg-indigo-950/30 rounded-full blur-[170px] pointer-events-none"
      />
      ...
      <motion.div
        ...
        transition={{ duration: 3, repeat: Infinity, repeatDelay: 6, ease: "easeOut" }}
        className="absolute top-16 left-12 w-20 h-[1px] bg-gradient-to-r from-amber-200 to-transparent blur-[0.5px] pointer-events-none"
      />
      ...
      <div className="pt-6 text-center space-y-2 z-10 max-w-lg">
        ...
        <h2 className="text-2xl sm:text-3xl font-normal text-rose-100 italic">
          {isCurrentCompleted
            ? `Constellation: ${currentConstellation.title}`
            : "Connect the breathing stars to reveal a memory"}
        </h2>
      </div>

      {/* PLANETARIUM CONSTELLATION CANVAS */}
      <div className="relative w-full max-w-xl aspect-square sm:aspect-[4/3] rounded-3xl bg-gradient-to-b from-[#0b0716] via-[#07040f] to-[#030208] border border-indigo-500/20 p-8 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.98)] my-auto flex flex-col items-center justify-between overflow-hidden">
        {/* Breathing Starfield Texture Overlay */}
        <div className="absolute inset-0 opacity-25 bg-[radial-gradient(#ffffff_1px,transparent_1px)] [background-size:24px_24px] pointer-events-none animate-pulse" />

        {/* Constellation Star Nodes Container */}
        <div
          onClick={handleConnectConstellation}
          className="relative w-full h-full cursor-pointer flex items-center justify-center"
        >
          {/* Constellation Connecting Line Glow */}
          {isCurrentCompleted && (
            <motion.svg
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
          ...
          <div className="relative flex flex-col items-center justify-center space-y-3 z-10 text-center">
            <motion.div
              animate={{
                scale: isCurrentCompleted ? 1.15 : [1, 1.1, 1],
                opacity: isCurrentCompleted ? 1 : [0.7, 1, 0.7],
              }}
            ...
            >
              <span>{currentConstellation.icon}</span>
            </motion.div>

            <p className="text-xs text-amber-300/90 font-mono tracking-widest pt-1">
              {isCurrentCompleted ? "Constellation Revealed" : "Tap to connect stars"}
            </p>
          </div>
        </div>

        {/* QUIET MEMORY UNFOLDING (BELOW CONSTELLATION - NO POPUPS) */}
        <AnimatePresence>
          {isCurrentCompleted && (
            <motion.div
              ...
              className="p-6 rounded-2xl bg-gradient-to-b from-[#140b15] to-[#09040a] border border-amber-500/25 shadow-2xl text-left space-y-3 max-w-lg w-full z-20"
            >
              <div className="flex items-center justify-between text-xs font-mono text-amber-300/80 border-b border-amber-500/15 pb-2">
                <span>{currentConstellation.date}</span>
                <span className="flex items-center gap-1 text-rose-300">
                  <MapPin size={12} /> {currentConstellation.location}
                </span>
              </div>
              <p className="font-serif text-sm sm:text-base text-rose-100/90 italic leading-relaxed">
